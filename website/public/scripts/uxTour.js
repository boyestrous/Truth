...
uxTour.prototype.setText = function (step) {
	...
	var element = document.getElementById(step.element),
		...
		marginLeft = (elementRect.width > elementRect.height ? elementRect.width : elementRect.height);
	...
	if (direction === 'top') {
		if (step.position === 'fixed') {
			...
			this.tooltip.style.top = (elementRect.top + window.pageYOffset  - tooltipRect.height - 50) + 'px';
		}
		...
		if (step.position === 'fixed') {
			this.tooltip.style.left = (elementRect.left - tooltipRect.width + 50) + 'px';
			console.log('left 1: ', this.tooltip.style.left);
		} else {
			this.tooltip.style.left = (elementRect.left - window.pageXOffset - .5*marginLeft) + 'px';
			this.tooltip.style.top = (elementRect.height/2 + elementRect.top - tooltipRect.height/2) + 'px';
			console.log('left 2: ', this.tooltip.style.left);
		}		
	} else {
		if (step.position === 'fixed') {
			this.tooltip.style.top = (elementRect.top + elementRect.height + 50) + 'px';
